...
/*   Updated: 2018/04/10 13:25:59 by yguaye           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				put_infos(t_mlx_context *ctx, t_window *win)
{
	...
	mlx_string_put(ctx->mlx, ctx->win, ctx->width - (ft_strlen(iter) * 10) - 2,
			18, 0x0066AAAA, iter);
	free(iter);
	mlx_string_put(ctx->mlx, ctx->win, 0, 0, 0x00FFFFFF, "Press ");
	mlx_string_put(ctx->mlx, ctx->win, 60, 0, 0x00CC9900, "R");
	mlx_string_put(ctx->mlx, ctx->win, 70, 0, 0x00FFFFFF, " to reset zoom");
